...
const ForgotPassword = () => {
  ...
  const handleEmailSubmit = async (e) => {
    ...
    try {
      ...
      setStep('otp');
      setOtp('');
      Swal.fire({ title: 'OTP Sent', text: res.data?.message || 'OTP sent to your email.', icon: 'success' });
    } catch (err) {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      setIsSubmitting(true);
      await verifyForgotPasswordOtpRequest({ email, otp });
      setStep('reset');
      Swal.fire({ title: 'OTP Verified', icon: 'success' });
    } catch (err) {
  ...
  return (
    <section className="min-h-[calc(100vh-8rem)] bg-slate-50 flex items-center justify-center px-4 py-10">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 sm:p-8 shadow-lg">
        ...
        <p className="mt-2 text-sm text-slate-600 text-center">
          ...
          {step === 'reset' && 'Set your new password to complete reset.'}
        </p>
