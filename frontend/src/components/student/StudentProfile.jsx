...
const StudentProfile = () => {
  ...
  const deleteAccount = async () => {
    ...
    try {
      ...
      const msg = err?.response?.data?.message || err.message || 'Error deleting account';
      alert(msg);
    }
  };
        ...
        <div className="bg-white shadow-lg shadow-gray-200/60 rounded-2xl overflow-hidden border border-gray-100">
          <div className="px-6 py-7 sm:p-8">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {['name', 'email', 'contactNumber', 'university'].map((key) => (
                <label key={key} className="block">
                  ...
                  <div className="mt-1.5 relative flex items-center gap-2 group">
                    {key === 'university' && editMode[key] ? (
                      <select
                        ref={refs[key]}
                        value={fields[key]}
                        onChange={(e) => onChange(key, e.target.value)}
                        className="
                          block w-full rounded-lg border-indigo-400 
                          shadow-sm 
                      ...
                      <input
                        ...
                        value={
                          key === 'university' && !editMode[key]
                            ? formatUniversityLabel(fields[key])
                            : fields[key]
                        }
                        onChange={(e) => onChange(key, e.target.value)}
